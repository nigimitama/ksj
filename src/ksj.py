...
def get_summary() -> pd.DataFrame:
    ...
    if error_msg != "正常に終了しました。":
        ...
        data_df = pd.io.json.json_normalize(data_dict["KSJ_SUMMARY_INF"]["KSJ_SUMMARY"]["item"])
    return data_df
...
def get_url(identifier, pref_code=None, mesh_code=None, metro_area=None, fiscal_year=None):
    ...
    params = {"appId": app_id, "lang": lang, "dataformat": data_format, "identifier": identifier}
    if pref_code:
    ...
    if error_msg != "正常に終了しました。":
        ...
        # convert dict to pd.DataFrame
        data_df = pd.io.json.json_normalize(data_dict["KSJ_URL_INF"]["KSJ_URL"]["item"])
    return data_df
...
def download_and_unzip(file_url, save_path, save_file_name=None):
    """
    指定したURLのzipファイルをダウンロードし、解凍する
    """
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    save_file_name = os.path.basename(file_url) if save_file_name is None else save_file_name
    save_file_name = os.path.join(save_path, save_file_name)
    urlretrieve(url=file_url, filename=save_file_name)
    # extract to save_dir
    with zipfile.ZipFile(save_file_name) as existing_zip:
